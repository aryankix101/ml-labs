...
def get_value_frequencies(feature):
    values = [datapoint for datapoint in feature]
    return {unique_value: values.count(unique_value) / len(values) for unique_value in set(values)}

def entropy(feature):
    return -1 * sum(outcome_frequency * math.log(outcome_frequency, 2) for outcome_frequency in get_value_frequencies(feature).values())

def select_subset(feature, value):
    return [datapoint for datapoint in feature[1:] if datapoint == value]

def e_entropy(feature):
    return sum(value_frequency * entropy(select_subset(feature, unique_value)) for unique_value, value_frequency in get_value_frequencies(feature).items())


nodes = generate_decision_tree(dataset, -1, [])
print_tree(nodes)
for feature in dataset[0:len(dataset)-1]:
    print(feature[0] + ": " + str(e_entropy(feature[1])))